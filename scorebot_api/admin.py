...
class AccessTokenForm(forms.ModelForm):
    ...
    access_gamemonitor_read = forms.BooleanField(
        required=False, label="GameMonitor.READ"
    )
    access_gamemonitor_update = forms.BooleanField(
        required=False, label="GameMonitor.UPDATE"
    )
    access_gamemonitor_create = forms.BooleanField(
        required=False, label="GameMonitor.CREATE"
    )
    access_gamemonitor_delete = forms.BooleanField(
        required=False, label="GameMonitor.DELETE"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if access_instance:
            ...
            for key_name in CONST_CORE_ACCESS_KEY_LEVELS.keys():
                ...
                access_keys[
                    "access_%s_%s" % (key_model[0].lower(), key_model[1].lower())
                ] = access_instance[key_name]
    ...
    def save(self, commit=True):
        ...
        for key_name in CONST_CORE_ACCESS_KEY_LEVELS.keys():
            if "_" in key_name:
                ...
                key_model = key_name.split(".")
            access_instance[key_name] = access_data.get(
                "access_%s_%s" % (key_model[0].lower(), key_model[1].lower())
            )
