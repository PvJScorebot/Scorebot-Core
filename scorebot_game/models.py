...
from scorebot_core.models import (
    Team,
    score_create_new,
    token_create_new,
    team_create_new_color,
    Credit,
    Token,
)
from scorebot.utils.constants import (
    CONST_GAME_GAME_RUNNING,
...
class GameModel(models.Model):
    ...
    def finished(self):
        api_debug(
            "BACKEND",
            'The class "%s" is not using the "finished" method to delete itself!'
            % self.__name__,
        )
...
class Job(GameModel):
    ...
    host = models.ForeignKey(
        "scorebot_grid.Host", on_delete=models.CASCADE, related_name="jobs"
    )
    ...
    def __str__(self):
        return "[Job] %s <%s> (%s)" % (
            ...
            self.host.get_canonical_name(),
            (
                "DONE"
                if self.finish is not None
                else "WAIT %s" % (timezone.now() - self.start).seconds
            ),
        )
    ...
    def is_expired(self, score_time):
        expired = self.finish is None and (score_time - self.start).seconds > int(
            self.monitor.game.get_option("job_timeout")
        )
        if expired:
    ...
    def can_cleanup(self, score_time):
        cleanup = self.finish is not None and (score_time - self.finish).seconds > int(
            self.monitor.game.get_option("job_cleanup_time")
        )
        if cleanup:
...
class Game(GameModel):
    ...
    scored = models.DateTimeField(
        "Game Last Scored", blank=True, null=True, editable=False
    )
    mode = models.SmallIntegerField(
        "Game Mode", default=0, choices=CONST_GAME_GAME_MODE_CHOICES
    )
    options = models.ForeignKey(
        "scorebot_core.Options", null=True, blank=True, on_delete=models.SET_NULL
    )
    status = models.PositiveSmallIntegerField(
        "Game Status", default=0, choices=CONST_GAME_GAME_STATUS_CHOICES
    )
    ...
    def get_json_scoreboard(self):
        game_json = {
            ...
            "events": [
                e.get_json_scoreboard() for e in GameEvent.objects.filter(game=self)
            ],
    ...
    def round_score(self, score_time):
        ...
        if self.scored is None or (score_time - self.scored).seconds > int(
            self.get_option("round_time")
        ):
            api_info("SCORING", 'Starting round scoring on Game "%s"..' % self.name)
            for team in self.teams.all():
                api_debug(
                    "SCORING", 'Scoring Team "%s" assets..' % team.get_canonical_name()
                )
    ...
    def reporter_check(self, check_time):
        ...
        if game_time > 3590 or game_time < 10:
            scores = [
                "%s: %d" % (t.name, t.score.get_score())
                for t in self.teams.filter(minimal=False)
            ]
            post_tweet(
                "%s %dm: Scores so far: %s"
                % (
                    self.name,
                    ((check_time - self.start).seconds % 3600) // 60,
                    ", ".join(scores),
                )
            )
            del scores
...
class GameTeam(GameModel):
    ...
    store = models.PositiveIntegerField(
        "Team Store ID", null=True, blank=True, unique=True
    )
    game = models.ForeignKey(
        "scorebot_game.Game", on_delete=models.CASCADE, related_name="teams"
    )
    players = models.ManyToManyField(
        "scorebot_core.Player", blank=True, related_name="game_teams"
    )
    token = models.ForeignKey(
        "scorebot_core.Token", on_delete=models.SET_NULL, blank=True, null=True
    )
    score = models.OneToOneField(
        "scorebot_core.Score", on_delete=models.SET_NULL, blank=True, null=True
    )
    beacons = models.ManyToManyField(
        "scorebot_core.Token", blank=True, editable=False, related_name="beacon_tokens"
    )
    ...
    def get_beacons(self):
        ...
        for beacon in self.compromises.filter(beacon__finish__isnull=True):
            beacons.append(
                {
                    "id": beacon.beacon.id,
                    "team": beacon.beacon.attacker.id,
                    "color": "#%s"
                    % hex(beacon.beacon.attacker.color).replace("0x", "").zfill(6),
                }
            )
        return beacons
...
class Purchase(GameModel):
    ...
    team = models.ForeignKey(
        "scorebot_game.GameTeam", on_delete=models.CASCADE, related_name="purchases"
    )
...
class GameEvent(GameModel):
    ...
    type = models.PositiveSmallIntegerField(
        "Event Type", choices=CONST_GAME_EVENT_TYPE_CHOICES, default=0
    )
...
class GameTicket(GameModel):
    ...
    team = models.ForeignKey(
        "scorebot_game.GameTeam", on_delete=models.CASCADE, related_name="tickets"
    )
    type = models.PositiveSmallIntegerField(
        "Ticket Category", default=0, choices=CONST_GRID_TICKET_CATEGORIES_CHOICES
    )

    def __str__(self):
        return "[Game Ticket] %s <%s|%d> %s" % (
            self.get_canonical_name(),
    ...
    def close_ticket(self):
        ...
        with atomic():
            ...
            if self.type == 1:
                ...
                api_score(
                    self.id, "TICKET-CLOSE", self.team.get_canonical_name(), self.total
                )
            else:
            ...
            api_info(
                "SCORING-ASYNC",
                'Team "%s" closed Ticket "%s" type "%s"!'
                % (self.team.get_canonical_name(), self.name, self.get_type_display()),
            )
            api_event(
                self.team.game,
                'Team %s just closed a Ticket "%s"!' % (self.team.name, self.name),
            )
            self.save()
    ...
    def reopen_ticket(self):
        ...
        with atomic():
            self.closed = False
            reopen_cost = float(
                float(self.team.game.get_option("ticket_reopen_multiplier")) / 100
            )
            score_value = -1 * (reopen_cost * self.total)
            self.team.set_tickets(score_value)
            api_score(
                self.id, "TICKET-REOPEN", self.team.get_canonical_name(), score_value
            )
            api_info(
                "SCORING-ASYNC",
            ...
            api_event(
                self.team.game,
                'Ticket "%s" for %s was reopened!' % (self.name, self.team.name),
            )
            self.save()
    ...
    def grab_ticket_json(request, json_data):
        if (
            "id" not in json_data
            or "name" not in json_data
            or "details" not in json_data
            or "type" not in json_data
            or "status" not in json_data
            or "team" not in json_data
        ):
            api_error(
                "TICKET",
                "Attempted to create a Ticket without the proper values!",
                request,
            )
        ...
        try:
            ...
            api_info(
                "TICKET",
                'Created a new Ticket with ID "%d"!' % ticket.ticket_id,
                request,
            )
        except GameTicket.MultipleObjectsReturned:
            api_error(
                "TICKET",
                'Multiple Tickets were returned with ID "%s", invalid!'
                % json_data["id"],
                request,
            )
            return None, "Invalid Ticket ID!"
        if not ticket_exists:
            ...
            api_info(
                "TICKET", 'Updating a Ticket with ID "%d"!' % ticket.ticket_id, request
            )
...
class GameMonitor(GameModel):
    ...
    selected_hosts = models.ManyToManyField(
        "scorebot_grid.Host", related_name="monitor", blank=True
    )

    def __str__(self):
        return "%s <%s:%d>" % (
            self.monitor.__str__(),
            ("I" if self.only else "E"),
            self.selected_hosts.all().count(),
        )
    ...
    def create_job(self):
        if self.game.status == CONST_GAME_GAME_RUNNING:
            ...
            for team_round in range(0, teams_max):
                ...
                api_debug(
                    "JOB",
                    'Monitor "%s" selected team "%s"!' % (self.monitor.name, team.name),
                )
                hosts = team.hosts.filter(scored__isnull=True)
                api_debug(
                    "JOB",
                    'Team "%s" has "%d" hosts to pick from!' % (team.name, len(hosts)),
                )
                for host_round in range(0, len(hosts)):
                    api_debug(
                        "JOB",
                        'Monitor "%s" start host selection round %d.'
                        % (self.monitor.name, host_round),
                    )
                    host = random.choice(hosts)
                    api_debug(
                        "JOB",
                        'Monitor "%s" attempting to select host "%s".'
                        % (self.monitor.name, host.fqdn),
                    )
                    if host.jobs.filter(finish__isnull=True).count() > 0:
                        api_debug(
                            "JOB",
                            'Host "%s" has currently open Jobs, moving on.' % host.fqdn,
                        )
                        del host
                        continue
                    if self.selected_hosts.all().count() > 0:
                        api_debug(
                            "JOB",
                            'Monitor "%s" has host selection rules in place. Type %s'
                            % (
                                self.monitor.name,
                                ("Include" if self.only else "Exclude"),
                            ),
                        )
                        ...
                        if host_rules == 0 and self.only:
                            api_debug(
                                "JOB",
                                'Monitor "%s" host selection rules denied host "%s".'
                                % (self.monitor.name, host.fqdn),
                            )
                            del host
                            del host_rules
                    ...
                    api_info(
                        "JOB",
                        'Gave Monitor "%s" Job "%d" for Host "%s".'
            ...
            api_debug(
                "JOB",
                'Monitor "%s" could not select any hosts, telling Monitor to wait.'
                % self.monitor.name,
            )
        return None
    ...
    def score_job(self, monitor, job, job_data):
        if self.monitor.id != monitor.id:
            api_error(
                "JOB",
                'Monitor "%s" returned a Job created by Monitor "%s"!'
                % (monitor.name, job.monitor.name),
            )
            return False, "Job was submitted by a different monitor!"
        api_info(
            "JOB", 'Processing Job "%d" send by Monitor "%s".' % (job.id, monitor.name)
        )
        if self.game.status != CONST_GAME_GAME_RUNNING:
            api_error(
                "JOB",
                'Job Game "%s" submitted by Monitor "%s" is not Running!'
                % (self.game, job.monitor.name),
            )
...
class GameCompromise(GameModel):
    ...
    checkin = models.DateTimeField(
        "Beacon Checkedin", null=True, blank=True, editable=False
    )
    attacker = models.ForeignKey(
        "scorebot_game.GameTeam",
        on_delete=models.CASCADE,
        related_name="attack_beacons",
    )

    def __len__(self):
        return (
            (self.finish - self.start).seconds
            if self.finish is not None
            else (timezone.now() - self.start).seconds
        )

    def __str__(self):
        return "[Beacon] %s -> %s (%d seconds)" % (
            self.attacker.name,
            self.host.get_fqdn(),
            self.__len__(),
        )
    ...
    def round_score(self):
        if self.__bool__():
            api_debug(
                "SCORING",
                'Beacon "%d" by "%s" is still on Host "%s"!'
            ...
            api_score(
                self.id,
                "BEACON",
                self.host.team.get_canonical_name(),
                beacon_value,
    ...
    def is_expired(self, now):
        if self.checkin is None:
            return (now - self.start).seconds > int(
                self.host.team.game.get_option("beacon_time")
            )
        return (now - self.checkin).seconds > int(
            self.host.team.game.get_option("beacon_time")
        )
...
class GameCompromiseHost(GameModel):
    ...
    ip = models.GenericIPAddressField(
        "Host Address", protocol="both", unpack_ipv4=True, null=True
    )
    team = models.ForeignKey(
        "scorebot_game.GameTeam", on_delete=models.CASCADE, related_name="compromises"
    )
    beacon = models.OneToOneField(
        "scorebot_game.GameCompromise", on_delete=models.CASCADE, related_name="host"
    )
    host = models.ForeignKey(
        "scorebot_grid.Host",
        on_delete=models.SET_NULL,
