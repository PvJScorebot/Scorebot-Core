...
class Team(models.Model):
    ...
    created = models.DateTimeField(
        "Team Registration", auto_now_add=True, editable=False
    )
    score = models.OneToOneField(
        "scorebot_core.Score", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class Token(models.Model):
    ...
    def __str__(self):
        return "[Token] %s - %s" % (
            self.uuid,
            (
                self.expires.strftime("%m/%d/%y %H:%M:%S")
    ...
    def __len__(self):
        return (
            max((self.expires - timezone.now()).seconds, 0)
            if self.expires is not None
            else 0
        )
...
class Score(models.Model):
    ...
    def __hash__(self):
        return self.id + self.get_score()

    def __str__(self):
        return "[Score] F:%d U:%d T:%d B:%d @ %s" % (
            self.flags,
            self.uptime,
            self.tickets,
            self.beacons,
...
class Options(models.Model):
    ...
    ticket_max_score = models.PositiveSmallIntegerField(
        "Ticket Max Score", default=6000
    )
    flag_stolen_rate = models.PositiveSmallIntegerField(
        "Flag Stolen Rate", default=8400
    )
    host_ping_ratio = models.PositiveSmallIntegerField(
        "Default Host Ping Percent", default=75
    )
    beacon_time = models.PositiveSmallIntegerField(
        "Default Beacon Timeout (seconds)", default=300
    )
    job_timeout = models.PositiveSmallIntegerField(
        "Unfinished Job Timeout (seconds)", default=300
    )
    ticket_reopen_multiplier = models.PositiveSmallIntegerField(
        "Ticket Reopen Multiplier", default=10
    )
    flag_captured_multiplier = models.PositiveSmallIntegerField(
        "Flag Captured Multiplier", default=300
    )
    job_cleanup_time = models.PositiveSmallIntegerField(
        "Finished Job Cleanup Time (seconds)", default=900
    )
    score_exchange_rate = models.PositiveIntegerField(
        "Score to Coin Exchange Rate Percentage", default=100
    )
    ticket_max_scoring = models.PositiveSmallIntegerField(
        "Ticket Max Scoring Time (seconds)", default=14400
    )
    ticket_grace_period = models.PositiveSmallIntegerField(
        "Ticket Scoring Grace Period (seconds)", default=900
    )
...
class Monitor(models.Model):
    ...
    access = models.ForeignKey(
        "scorebot_core.AccessToken", on_delete=models.SET_NULL, null=True, blank=True
    )
...
class AccessToken(models.Model):
    ...
    token = models.ForeignKey(
        "scorebot_core.Token", on_delete=models.SET_NULL, null=True, blank=True
    )

    def __str__(self):
        return "[Access] %s <%d>" % (
            (self.token.uuid if self.token is not None else "(null)"),
            self.level,
        )
    ...
    def __getitem__(self, access_level):
        if isinstance(access_level, int):
            ...
            raise TypeError(
                'Parameter "access_level" must be a "integer" or "string" type!'
            )
        if not self.token:
            return False
