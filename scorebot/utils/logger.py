...
class Logger(object):
        ...
        if not os.path.isdir(log_directory):
            if not os.path.exists(log_directory):
                try:
                    ...
                    raise OSError(
                        'Parameter "log_directory" is not a directory! (%s)'
                        % str(osError)
                    )
    ...
    def setup_log(
        self, log_name, log_level=LOGGER_DEFAULT_LEVEL, log_format=LOGGER_DEFAULT_FORMAT
    ):
        log_handler = logging.FileHandler(
            os.path.join(self.log_dir, "%s.log" % log_name.lower())
        )
...
def log_info(log_name, log_message):
    if LOGGER_INSTANCE is None:
        print(
            "[!] Logger is not initialized yet! Please initialize it!\n[+] %s"
            % log_message
        )
...
def log_stdout(
    log_name, log_level=LOGGER_DEFAULT_LEVEL, log_format=LOGGER_DEFAULT_FORMAT
):
...
def init_simple(log_file, log_level="INFO"):
    simple_config = {
        ...
        "loggers": {
            "": {"handlers": ["default_file"], "level": log_level, "propagate": True}
        },
    }
