...
def authenticate(requires=None):
    def _authenticate_wrapper(auth_function):
        def _authenticate_wrapped(*args, **kwargs):
            ...
            if http_request is None:
                log_error(
                    "API", "AUTH (NO-IP): Connected but no HTTP headers were found!"
                )
            ...
            if "HTTP_SBE_AUTH" not in http_request.META:
                ...
                return HttpResponseForbidden(
                    content='{"message": "SBE API: Missing authentication header!"}'
                )
            try:
                ...
                return HttpResponseForbidden(
                    content='{"message": "SBE API: Invalid authentication token format!"}'
                )
                ...
                return HttpResponseForbidden(
                    content='{"message": "SBE API: Invalid authentication token!"}'
                )
            ...
            if not http_request.authentication:
                ...
                return HttpResponseForbidden(
                    content='{"message": "SBE API: Authentication token is expired!"}'
                )
...
def dump_data(dump_name, data_to_dump):
    ...
    if settings.DUMP_DATA:
        try:
            ...
            dump_file = open(
                "dump-%s-%s"
                % (dump_name, timezone.now().strftime("%m.%d.%y-%H.%M.%S")),
                "w",
            )
...
def import_all_models(models_class, models_ignore=list()):
    ...
    for model_class_object in inspect.getmembers(models_class, inspect.isclass):
        if (
            model_class_object is not None
            and model_class_object[1] not in _MODELS_REGISTERED
        ):
            if (
                model_class_object not in models_ignore
                or model_class_object.__name__ not in models_ignore
            ):
                if (
                    issubclass(model_class_object[1], models.Model)
                    and not model_class_object[1]._meta.abstract
                ):
                    if "django.contrib" not in model_class_object[1].__module__:
                        log_debug(
                            "BACKEND",
                            'Registering model "%s"' % model_class_object[1].__name__,
                        )
                        try:
                            admin_model = getattr(
                                sbe_admin, "%sAdmin" % model_class_object[1].__name__
                            )
...
def game_team_from_token(
    request, api_name, json_field, offense=True, beacon=False, fields=None
):
    ...
    try:
        ...
        log_error(
            "API",
            "%s (%s): Data submitted by is not encoded properly!"
    ...
    try:
        ...
        log_error(
            "API",
            "%s (%s): Data submitted is not in a valid JSON format!"
            % (api_name.upper(), client),
        )
        return (
            None,
    ...
    if json_field not in json_data:
        log_error(
            "API",
            '%s (%s): Data submitted is missing field "%s"!'
    ...
    try:
        ...
        log_error(
            "API",
            "%s (%s): Team Token submitted is not a valid format!"
            % (api_name.upper(), client),
        )
        return (
            None,
        ...
        log_error(
            "API",
            "%s (%s): Team Token submitted does not exist!"
            % (api_name.upper(), client),
        )
        return (
            None,
    ...
    if not token.__bool__():
        log_error(
            "API",
            "%s (%s): Team Token submitted is expired!" % (api_name.upper(), client),
        )
        return (
            None,
            None,
    ...
    try:
        ...
        log_error(
            "API",
            "%s (%s): Token submitted is not linked to a Team!"
            % (api_name.upper(), client),
        )
        return (
            None,
            None,
            None,
            HttpResponseForbidden(content='{"message": "SBE API: Not a Team Token!"}'),
        )
    ...
    log_debug(
        "API",
        '%s (%s): Team "%s" was authenticated to the API!'
    ...
    if team.game.status != CONST_GAME_GAME_RUNNING:
        log_error(
            "API",
            '%s (%s): Game "%s" submitted is not Running!'
